...
def analyze_sentiments_batch(client, comments, model="mixtral-8x7b-32768", temperature=0.7, batch_size=50):
    """
    Analyzes the sentiments of a list of comments in batches using the Groq API.

    This function sends comments to the Groq API in batches, requesting sentiment analysis for each comment in the context
    of fashion. It returns a list of sentiments ('positive', 'neutral', or 'negative') corresponding to the comments.

    Parameters:
    - client: The Groq API client instance.
    - comments (list of str): A list of comments to analyze.
    - model (str): The model to use for sentiment analysis. Default is "mixtral-8x7b-32768".
    - temperature (float): The sampling temperature for the API request. Default is 0.7.
    - batch_size (int): The number of comments to process in each batch. Default is 50.

    Returns:
    - list of str: A list of sentiments ('positive', 'neutral', or 'negative') corresponding to each comment.
      Returns None if an error occurs during the analysis.
    """
    ...
    for i in range(0, len(comments), batch_size):
        ...
        if len(batch_sentiments) != len(batch_comments):
            st.error("Mismatch in number of sentiments returned for a batch.")
            return None
    ...
    if len(batched_sentiments) != len(comments):
        st.error("Total mismatch in number of comments and sentiments.")
        return None
    ...
    if "df" in st.session_state and not st.session_state.df.empty:
        ...
        if model:
            ...
            if st.button("Analyze Sentiments"):
                ...
                # Debugging: Print the clean comments before sending to sentiment analysis
                # st.write("Clean Comments for Sentiment Analysis:", clean_comments)

                sentiments = analyze_sentiments_batch(client, clean_comments, model)
                if sentiments:
                    # Debugging: Print the sentiments to ensure they match the comments
                    # st.write("Sentiments:", sentiments)

                    if len(clean_comments) == len(sentiments):
                        comment_sentiment_pairs = list(zip(clean_comments, sentiments))

                        st.session_state.df["sentiments"] = str(sentiments)
                        st.write("Sentiments Analysis:")
                        
                        # Display of comments and sentiments
                        for comment, sentiment in comment_sentiment_pairs:
                            st.markdown(
                                f"""
                                <div style="background-color: #f9f9f9; padding: 10px; margin: 10px 0; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
                                    <p><strong>Comment:</strong> {comment}</p>
                                    <p><strong>Sentiment:</strong> <span style="color: {'green' if sentiment == 'positive' else 'red' if sentiment == 'negative' else 'grey'};">{sentiment}</span></p>
                                </div>
                                """, 
                                unsafe_allow_html=True
                            )
                        ...
                        st.write("Error: Mismatch in number of comments and sentiments")
